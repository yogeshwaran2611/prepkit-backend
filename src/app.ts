...
import { FileCache, GeminiProvider, HttpFetcher, defaultSearch } from './utils/providers/index.js';
...
import { JobRunner } from './services/jobRunner.service.js';
...
export function createApp(config: Config, db: Db): App {
  ...
  app.use('/api/auth', authRoutes(config, db));
  app.use('/api/kits', kitsRoutes(config, db, runner));
  app.use('/api/jobs', jobsRoutes(config, db, runner));
