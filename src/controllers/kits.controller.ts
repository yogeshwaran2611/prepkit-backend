...
/**
 * All the main logic for the /api/kits endpoints. Routes (src/routes/kits.routes.ts) stay
 * thin — they only wire an HTTP verb + path + middleware to one of these functions.
 *
 * A factory rather than free functions with an imported singleton `db`: the same controller
 * code runs against the real MongoDB service and against the JSON-file service in tests,
 * with no environment-specific branching inside the handlers themselves.
 */
export function createKitsController(db: Db, runner: JobRunner) {
  return {
    ...
    async createBatch(req: AuthedRequest, res: Response): Promise<void> {
    ...
    async patchItem(req: AuthedRequest, res: Response): Promise<void> {
      const { patch } = req.body as PatchItemBody;
      const itemId = req.params.itemId!;

      const updated = await db.kits.patchKit(req.user.id, req.params.id!, (kit) => applyItemPatch(kit, itemId, patch));
    ...
    async reorder(req: AuthedRequest, res: Response): Promise<void> {
    ...
    async addItem(req: AuthedRequest, res: Response): Promise<void> {
    ...
    async deleteItem(req: AuthedRequest, res: Response): Promise<void> {
      const itemId = req.params.itemId!;
      const updated = await db.kits.patchKit(req.user.id, req.params.id!, (kit) => {
    ...
    async regenerate(req: AuthedRequest, res: Response): Promise<void> {
    ...
    async recordPractice(req: AuthedRequest, res: Response): Promise<void> {
    ...
    async listPractice(req: AuthedRequest, res: Response): Promise<void> {
      const record = await db.kits.findOwned(req.user.id, req.params.id!);
      if (!record) throw ApiError.notFound('That kit does not exist.');
      const events = await db.practice.listForKit(req.user.id, record.id);
      res.json({ data: { events: events.map((e) => ({ cardId: e.cardId, confidence: e.confidence, at: e.at })) } });
...
export type KitsController = ReturnType<typeof createKitsController>;
