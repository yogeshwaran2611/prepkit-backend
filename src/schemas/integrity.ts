...
export type IntegrityCode =
  ...
  | 'DAY_COUNT_MISMATCH'
  | 'DAY_SEQUENCE_GAP'
  | 'DUPLICATE_FIRST_ASSIGNMENT'
  | 'REPEAT_ON_NEW_DAY'
  | 'QUESTION_UNSCHEDULED'
  | 'MUST_WITHOUT_QUESTION'
  | 'MUST_UNSCHEDULED'
  | 'MUST_REPORTED_UNCOVERED'
  | 'DUPLICATE_ID';
...
export function checkKitIntegrity(kit: Kit): IntegrityIssue[] {
  ...
  const push = (code: IntegrityCode, message: string, ids?: string[]) =>
    issues.push(ids ? { code, message, ids } : { code, message });
  ...
  if (dupes.length) push('DUPLICATE_ID', 'ids must be unique within a kit', unique(dupes));
  ...
  if (days.length !== kit.schedule.days_available)
    push(
      'DAY_COUNT_MISMATCH',
      `schedule has ${days.length} days, days_available is ${kit.schedule.days_available}`,
    );
  days.forEach((d, i) => {
    if (d.day !== i + 1)
  ...
  const firstSeenOn = new Map<string, number>();
  for (const d of days) {
    for (const id of d.question_ids) {
      ...
      if (seen === undefined) {
        ...
        push(
          'REPEAT_ON_NEW_DAY',
          `question ${id} repeats on new-day ${d.day} (first assigned day ${seen}); repeats are legal only on review days`,
          [id],
        );
      }
    }
  ...
  for (const r of musts) {
    if (!coveredReqIds.has(r.id))
      ...
      const scheduled = kit.questions.some(
        (q) => q.requirement_ids.includes(r.id) && firstSeenOn.has(q.id),
      );
      if (!scheduled)
    ...
    if (kit.coverage.uncovered_requirement_ids.includes(r.id))
      push(
        'MUST_REPORTED_UNCOVERED',
        `must-have ${r.id} is reported uncovered; the fallback question path should have closed it`,
        [r.id],
      );
  }
...
export type ValidateResult =
  | { ok: true; kit: Kit }
  | { ok: false; zodErrors: string[]; issues: IntegrityIssue[] };
